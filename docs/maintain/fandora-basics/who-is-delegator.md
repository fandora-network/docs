...
The FANDORA tokens are bonded with the next [checkpoint](/docs/maintain/glossary#checkpoint-transaction) committed on the Ethereum mainnet. Delegators also have an option to opt out of the system whenever they want. Similar to validators, delegators have to wait for the unbonding period, which consists of approximately 9 days, to end before withdrawing their stake.
...
the fandora network. Each delegator is responsible for managing their own risk, but in doing so, delegators 
