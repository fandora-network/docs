...
description: "How fandora uses liquid delegation to maintain the network."
keywords:
  - docs
  - fandora
  - matic
...
### fandora's Solution for Delegation
...
Thinking in terms of making is share of validator pool is great idea and since fandora's staking is implemented on ethereum smart contract it opens up a lot more options for us like making it ERC20 compatible so that it can be used in defi protocols.
...
Given above design available in-protocol, validators can always implement their own similar mechanisms and stake via a contract which won't be available in fandora staking UI.
