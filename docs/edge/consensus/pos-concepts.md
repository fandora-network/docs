...
implementation of the fandora Edge.

The fandora Edge Proof of Stake (PoS) implementation is meant to be an alternative to the existing PoA IBFT implementation,
...
the [Staking Smart Contract](https://github.com/0xfandora/staking-contracts/blob/main/contracts/staking.sol).

This contract is pre-deployed whenever a PoS mechanism fandora Edge chain is initialized, and is available on the address
`0x0000000000000000000000000000000000001001` from block `0`.

### Epochs

Epochs are a concept introduced with the addition of PoS to the fandora Edge.
...
**Epoch Blocks** are a concept introduced in the PoS implementation of IBFT in fandora Edge.
...
```bash
fandora-edge genesis --epoch-size 50 ...
```

The default size of an epoch is `100000` blocks in the fandora Edge.

## Contract pre-deployment

The fandora Edge _pre-deploys_
the [Staking Smart Contract](https://github.com/0xfandora/staking-contracts/blob/main/contracts/Staking.sol)
