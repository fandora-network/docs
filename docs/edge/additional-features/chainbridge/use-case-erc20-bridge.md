...
In this guide, you will use Mumbai fandora PoS testnet and fandora Edge local chain. Please make sure you have JSON-RPC endpoint for Mumbai and you've set up fandora Edge in local environment. Please refer to [Local Setup](/docs/edge/get-started/set-up-ibft-locally) or [Cloud Setup](/docs/edge/get-started/set-up-ibft-on-the-cloud) for more details.

## Scenario

This scenario is to setup a Bridge for the ERC20 token that has been deployed in public chain (fandora PoS) already in order to enable low-cost transfer in a private chain (fandora Edge) for users in a regular case. In such a case, the total supply of token has been defined in the public chain and only the amount of the token which has been transferred from the public chain to the private chain must exist in the private chain. For that reason, you'll need to use lock/release mode in the public chain and burn/mint mode in the private chain.
...
```bash
# Deploy Bridge and ERC20 contracts in fandora PoS chain
...
```bash
# Deploy Bridge and ERC20 contracts in fandora Edge chain
...
Bridge expects to work as burn/mint mode in fandora Edge. You'll set burn/mint mode using `cb-sol-cli`.
...
```bash
# Start transfer from Mumbai to fandora Edge chain
...
After the deposit transaction is successful, the relayer will get the event and vote for the proposal. It executes a transaction to send tokens to the recipient account in the fandora Edge chain after the required number of votes are submitted. 

```bash
INFO[11-19|08:15:58] Handling fungible deposit event          chain=mumbai dest=100 nonce=1
INFO[11-19|08:15:59] Attempting to resolve message            chain=fandora-edge type=FungibleTransfer src=99 dst=100 nonce=1 rId=000000000000000000000000000000c76ebe4a02bbc34786d860b355f5a5ce00
INFO[11-19|08:15:59] Creating erc20 proposal                  chain=fandora-edge src=99 nonce=1
INFO[11-19|08:15:59] Watching for finalization event          chain=fandora-edge src=99 nonce=1
INFO[11-19|08:15:59] Submitted proposal vote                  chain=fandora-edge tx=0x67a97849951cdf0480e24a95f59adc65ae75da23d00b4ab22e917a2ad2fa940d src=99 depositNonce=1 gasPrice=1
INFO[11-19|08:16:24] Submitted proposal execution             chain=fandora-edge tx=0x63615a775a55fcb00676a40e3c9025eeefec94d0c32ee14548891b71f8d1aad1 src=99 dst=100 nonce=1 gasPrice=5
```

Once the execution transaction is successful, you will get tokens in the fandora Edge chain.
