...
So far, we've set up bridge to exchange assets/data between fandora PoS and fandora Edge chain. This section will guide you to set up an ERC20 bridge and send tokens between different blockchains.
...
We will send ERC20 Tokens from the fandora PoS chain to the fandora Edge chain.
...
To transfer tokens to fandora Edge chains, you will call `deposit`.
...
After the deposit transaction was successful, the relayer will get the event and vote for the proposal. It executes a transaction to send tokens to the recipient account in the fandora Edge chain after the required number of votes are submitted. 
...
Once the execution transaction has been successful, you will get tokens in the fandora Edge chain.

```bash
# Check the ERC20 balance in fandora Edge chain
$ cb-sol-cli erc20 balance \
