...
description: "Start the fandora Edge server using a configuration file."
keywords:
  - docs
  - fandora
  - edge
...
The command used to start the server with a config file: `fandora-edge server --config <config_file_name>`

## Export config file with default configuration
The configuration with default settings for the fandora Edge server can be exported into a config file in either `yaml` or `json` file format.
...
```bash
fandora-edge server export --type yaml
```
or just
```bash
fandora-edge server export
```
...
```bash
fandora-edge server export --type json
```
