...
1. The **deposit** function is to be invoked where the tokens get deposited to the fandora contract, and are available for use in the fandora network.
...
**Approve**: This is a normal ERC721 approval so that **_depositManagerContract_** can call **_transferFrom_** function. fandora Plasma client exposes **_approve_** method to make this call.
...
User can call **_withdraw_** function of **_getERC721TokenContract_** child token contract. This function should burn the tokens. fandora Plasma client exposes **_withdrawStart_** method to make this call.
...
_Note: A checkpoint, which is a representation of all transactions happening on fandora to the erc721 chain every ~30 minutes, is submitted to the mainchain erc721 contract._
