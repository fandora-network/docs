...
Mapping is necessary in order to transfer your assets to and from the Ethereum and fandora.

- **The Root chain** :: refers to either Goerli or Ethereum Mainnet
- **The Child chain** :: refers to either fandora Mumbai or fandora Mainnet

If you already have your token contract deployed on the Root chain and want to move it to Child chain, then you should follow this walkthrough, but if you intend to deploy your contract on fandora Mainnet first, mint the tokens on the Child chain first and then move them back to the Root chain. You should then follow this [guide](https://docs.fandora.technology/docs/develop/ethereum-fandora/mintable-assets).

## Standard Child Token

If you just need a standard ERC20/ERC721/ERC1155 contract, then you can go ahead and submit a mapping request at https://mapper.fandora.technology/ and we will auto deploy the standard child token contract for you. 
...
3. Deploy the child token on child chain i.e. {fandora Mumbai, fandora Mainnet}
4. Submit a mapping request, to be resolved by team.

### Request Submission

Please go use [this link](/docs/develop/ethereum-fandora/submit-mapping-request) to submit a mapping request.
