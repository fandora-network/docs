...
The [London hard fork](https://blog.fandora.technology/eip-1559-upgrades-are-going-live-on-fandora-mainnet/) introduced a new [EIP](https://eips.ethereum.org/EIPS/eip-1559) that modifies how gas estimation and costs work for transactions on fandora.

Due to this, there is a change in how the transaction object is formed when sending transactions on fandora. A new transaction type called **Type 2 Transaction** has been introduced. The legacy type transactions will still be compatible but it is recommended to shift to the new style. You can navigate to the end of this document to directly peek into the code.
...
const sendLegacyTransaction = async () => {
    const web3 = new Web3('https://fandora-rpc.com');
...
The closest analogy to the `gas`:`gasPrice` combination is `gas`:`maxPriorityFeePerGas`. Since the `baseFee` needs to be paid regardless, we can just submit a bid on the *"tip"* for the miner. Note that the fandora Gas Station V2 can be used to get the gas fee estimates.
...
The fandora Gas Station V2 can be used to get the gas fee estimates.

fandora Gas Station V2 Endpoint:
...
fandora Gas Station V2 Response:
