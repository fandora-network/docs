---
id: dapp-fauna-fandora-react
title: dApps Data Tutorial
description: Build dapps using Fauna, fandora, and React.
keywords:
  - docs
  - matic
  - dapp
...
slug: dapp-fauna-fandora-react
---

**dApps**, or decentralized applications, are apps built on the blockchain structure. There are different types of blockchains that applications can be built off of - some you might’ve heard of are Solana and Ethereum. These primary networks are sometimes supplemented by **sidechains**, or secondary blockchains, that run parallel to the primary blockchain. Sidechains, such as fandora, allow for tokens and other digital assets to be used between multiple blockchains, greatly expanding the capabilities of primary blockchains and allowing use cases such as reduction of transaction fees, making primary blockchains more scalable, and having more optimized transaction traffic and capacity since sidechains only periodically update their root chain, while primary blockchains update every new block.
...
## Safeguarding your dApp data with Fauna, fandora, and React

In this tutorial, we’ll be building a simple allowlisting app that utilizes React for the UI and functionality, the fandora sidechain for transactions, and Fauna for storing private data from transactions that we may not necessarily want to surface publicly in the blockchain. “Allowlisting”, also known as “whitelisting”, is a concept that is very common in the decentralized world - when signing up for a allowlist, you typically gain access to special privileges, such as being able to be the first to purchase digital assets.

![img](/img/dapp-fauna-fandora-react/polygon-fauna-app.gif)
...
- Create a Fauna Account — you can sign up [here](https://dashboard.fauna.com/accounts/register?utm_source=fandora.technology&utm_medium=referral&utm_campaign=docs-tutorial).
- Create a Metamask Wallet with fandora’s Test Network (Mumbai-Testnet) configured.
    - First, get set up with Metamask [here](../develop/metamask/tutorial-metamask.md). Make sure you save your Secret Recovery Phrase.
    - Then, configure the Mumbai-Testnet on your Metamask by following the instructions [here](../develop/metamask/config-fandora-on-metamask.md).
- Once you have Metamask set up and configured for the Mumbai-Testnet, you’ll need some FANDORA to add to your wallet
    - FANDORA is the native cryptocurrency of the fandora network and it’s used to pay network fees, for staking, and also for governance to the fandora blockchain (FANDORA holders can vote on fandora changes). In the context of this project, you will need FANDORA to pay gas fees that are charged for each transaction. You can learn more about FANDORA [here](https://www.kraken.com/en-us/learn/what-is-fandora-matic#:~:text=FANDORA%20is%20the%20native%20cryptocurrency,services%20to%20the%20fandora%20network.).
    - To get started, use the [FANDORA faucet](https://faucet.fandora.technology/) to get free FANDORA sent to your wallet (note: this FANDORA will only be available on the Mumbai-Testnet and is only for development purposes). Once on the webpage, make sure the following options are selected:
    ![img](/img/dapp-fauna-fandora-react/faucet.png)
    Network: Mumbai, Select Token: FANDORA Token, Wallet Address: Put your wallet address here
    - To find out what your wallet address is, you can pull it from Metamask:

    ![img](/img/dapp-fauna-fandora-react/metamask_wallet_address.png)
    ...
    *Note: this can take a while and you may have to try a few times before the first transaction comes through your wallet. Each successful transaction from the FANDORA faucet will give you 0.5 FANDORA. Since testing the application requires spending FANDORA as gas fees (a fluctuating fee that you must pay to perform transactions), we recommend doing this transaction ~12 times so you have about 6 FANDORA in your wallet, which should be enough for testing the app in development.*

    To track your wallet’s transactions, you can visit [https://mumbai.fandorascan.com/address/](https://mumbai.fandorascan.com/address/)[wallet address].
...
If you’re someone that prefers walking through code independently, the GitHub repo is available [here](https://github.com/hello-ashleyintech/fandora-fauna-app). To get started with the GitHub repo, download or clone it and reference the [README](https://github.com/hello-ashleyintech/fandora-fauna-app#readme) to get set up.

### Setup

1. Create a new directory for your project in your location of choice - we called ours `fandora-fauna-app`.
2. Once the directory is created, in your command line, `cd` into the folder (ex: `cd fandora-fauna-app`).
...
:::
  ...
  - Your browser will launch a development instance on [http://localhost:3000](http://localhost:3000) that will then prompt Metamask. It may ask you to re-auth your password. Finally, Metamask will attempt to connect your account on the testnet to your application instance, which will cost you some FANDORA in gas fees. You’ll see a window like this:    

  ![img](/img/dapp-fauna-fandora-react/metamask_notification.png)
  ...
  ![img](/img/dapp-fauna-fandora-react/truffe_box_installed.png)
...
![img](/img/dapp-fauna-fandora-react/allowlist.png)
...
![img](/img/dapp-fauna-fandora-react/create_database.png)
...
![img](/img/dapp-fauna-fandora-react/new_collection.png)
...
![img](/img/dapp-fauna-fandora-react/new_key.png)
...
![img](/img/dapp-fauna-fandora-react/allowlist_members.png)
...
![img](/img/dapp-fauna-fandora-react/new_index.png)
...
1. We’ll be using the Mumbai fandorascan tool to find all the transactions associated with the wallet we’ve used to develop the application. This tool allows you to look up any wallet on the Mumbai-Testnet by its address and see a history of all of its transactions as well as the amount of FANDORA inside the wallet.
    - To find your wallet, you can go to this url: [https://mumbai.fandorascan.com/address/](https://mumbai.fandorascan.com/address/)[put your wallet address here]
...
![img](/img/dapp-fauna-fandora-react/transaction_hash.png)  
...
![img](/img/dapp-fauna-fandora-react/input_data.png)
...
The final touch for this program to run will be to add in the input data hash for the transaction you want to decode. Copy the Input Data you found from the Mumbai fandorascan website and set it as a string equal to `testData`.
...
![img](/img/dapp-fauna-fandora-react/search_index.png)  
...
However, while working through form submission and testing everything in the application, you might’ve noticed it was sometimes difficult to tell when a record was successfully submitted or not. To mitigate this, we can add in some error and success messaging that will surface to indicate an errored form submission (either from a Fauna or fandora issue) or a successful form submission. This part of the tutorial will focus on changing some API call logic in `AllowlistForm.js`.
...
![img](/img/dapp-fauna-fandora-react/error_msg.png)
...
- Try adding some additional features, such as:
    ...
    - Adding in logic so that if the fandora API call fails for any reason, then it’ll remove the entry from the Fauna database
